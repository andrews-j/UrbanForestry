...
# alive_trees <- data.frame(Mortality_2023 = "Alive", Count = sum(alive_trees))
# dead_trees <- data.frame(Mortality_2023 = "Dead", Count = sum(dead_trees))
# unknown_trees <- data.frame(Mortality_2023 = "Unknown", Count = sum(unknown_trees))
# 
...
#   group_by(Mortality_2023) %>%
#   mutate(Percentage = round((Count / sum(tree_counts$Count)) * 100), 1)
# 
# # Create the bar graph
# ggplot(tree_counts, aes(x = Mortality_2023, y = Count, fill = Mortality_2023)) +
#   geom_bar(stat = "identity") +
