...
alltreedata <- read.csv("Z:\\HERO_2023\\Fall_DCR_Research\\Data\\2014-15_Baseline_with_tree_categories_9_13.csv")
...
ggplot(organized_species_df, aes(x = Species, y = Count, fill = Status)) +
  ...
  geom_text(aes(label = ifelse(Count > 5, Count, "")), position = position_stack(vjust = 0.5), color = "black", size = 3) +
  annotate("text", x = Inf, y = Inf, label = paste("Total Count:", sum(organized_species_df$Total_count)/3), hjust = 1, vjust = 2, size = 5, color = "black") +
...
ggplot(organized_species_df, aes(x = Species, y = Count, fill = Status)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = ifelse(Count > 0, Count, "")), position = position_stack(vjust = 0.5), color = "black", size = 3) +
  annotate("text", x = Inf, y = Inf, label = paste("Total Count:", print_count_sans), hjust = 1, vjust = 2, size = 5, color = "black") +
  labs(title = "", x = "Species", y = "Count") +
  scale_fill_manual(
    values = c(Count_alive = "#4DAC19", Count_dead = "#F3A2B9", Count_unknown = "gray"),
    labels = c(Count_alive = "Alive", Count_dead = "Dead", Count_unknown = "Unknown")
...
#graph survivorship by species sans Arborvitae and White Fir
combined_sans_arbor_fir <- filter(combined_species_df_long, Species != 'American Arborvitae' & Species != 'White Fir' & Total_count >= 10)
#total_count <- sum(combined_sans_arbor_fir$Total_count)

ggplot(combined_sans_arbor_fir, aes(x = Species, y = Count, fill = Status)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = ifelse(Count > 2, Count, "")), position = position_stack(vjust = 0.5), color = "black", size = 3) +
  annotate("text", x = Inf, y = Inf, label = paste("Total Count:", total_count), hjust = 1, vjust = 2, size = 5, color = "black") +
  labs(title = "", x = "Species", y = "Count") +
  scale_fill_manual(
    values = c(Count_alive = "#4DAC19", Count_dead = "#F3A2B9", Count_unknown = "gray"),
    labels = c(Count_alive = "Alive", Count_dead = "Dead", Count_unknown = "Unknown")) +
...
"__________________________________________________________________________________________________________"
#ALL Species ranked by percentage survivorship
#NOTES::: Color by above, at, or below average survivorship. Average survivorship w/o arborvitae?


# Filter the combined_species_df for species with a minimum count of 15
filtered_species_df_all <- combined_species_df 
filtered_species_df_all <- filter(filtered_species_df_all, Species != 'American Arborvitae' & Species != 'White Fir' & Total_count >= 15)
...
ggplot(ordered_species_df_all, aes(x = Species, y = Percentage_survivorship, fill = Species)) +
  ...
  annotate("text", x = Inf, y = Inf, label = paste("Total Count:", sum(ordered_species_df_all$Total_count)), hjust = 1, vjust = 1, size = 4, color = "black")

mean(ordered_species_df_all$Total_count)
...
ggplot(ordered_species_df_sans, aes(x = Species, y = Percentage_survivorship, fill = Species)) +
  geom_bar(stat = "identity", fill = "#4DAC19") +
  geom_text(aes(label = ifelse(Percentage_survivorship > 0, paste0(Total_count), "")),
...
alive_trees <- data.frame(Mortality = "Alive", Count = nrow(alive_trees))
dead_trees <- data.frame(Mortality = "Dead", Count = nrow(dead_trees))
unknown_trees <- data.frame(Mortality = "Unknown", Count = nrow(unknown_trees))
